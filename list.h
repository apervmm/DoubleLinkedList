...
List<Type>& List<Type>::operator=(const List<Type>& other) {
	...
	for (Node* i = other.head; i != nullptr; i = i->next) //push_back
	{
		if (head == nullptr)
		{
			head = tail = new Node(i->item, nullptr, nullptr);
...
void List<Type>::push_back(const Type& item)
{
	if (this->empty())
	{
		this->push_front(item); //calling pushfront
	}
	else
	{
		tail = tail->next = new Node(item, tail, nullptr);
		num_nodes++;//inside because i am calling push front if it is empty
	}
}
...
void List<Type>::add_at(int index, const Type& item) {
	//cases when i need to add before or after
	if (index <= 0)
	{
...
bool List<Type>::pop_front()
{
	...
	head->prev = nullptr;// prev of head  = nullptr
...
bool List<Type>::pop_rear() {
	...
	delete rear;
	num_nodes--;
	tail->next = nullptr; //last element = nullptr
